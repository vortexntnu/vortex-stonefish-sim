...
public:
  VortexSimInterface() : Node("vortex_sim_interface") {
    ...
    subscription_ = this->create_subscription<vortex_msgs::msg::ThrusterForces>(
        "orca/thruster_forces", qos_sensor_data,
    ...
    odom_euler_pub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
        "/nucleus/odom_euler", qos_sensor_data);

    pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
        "/nucleus/pose", qos_sensor_data);
...
private:
  ...
  void
  thruster_callback(const vortex_msgs::msg::ThrusterForces::SharedPtr msg) {
    if (msg->thrust.size() != 8) {
      RCLCPP_ERROR(
          this->get_logger(),
          "Received ThrusterForces with incorrect size. Expected 8, got %zu",
          msg->thrust.size());
      return;
    }
  ...
  // void publish_odom_height_tf(const nav_msgs::msg::Odometry::SharedPtr
  // odom_msg)
  // {
  ...
  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr odom_msg) {
    ...
    // transform_stamped1.transform.translation.z =
  ...
  void publish_world_odom_tf() {
    ...
    // Create the NED to ENU transformation (90 deg rotation about Z, then 180
    // deg rotation about X)
    tf2::Quaternion q1;
    q1.setRPY(M_PI, 0.0, M_PI_2); // First rotate by 180 degrees about X, then
                                  // -90 degrees about Z
